...
public class Z7PainkillersItem extends Item {
//    public static final StatusEffectInstance EFFECT_HEALTH = new StatusEffectInstance(StatusEffects.INSTANT_HEALTH, 1, 1);
    ...
    public Z7PainkillersItem() {
        super(new FabricItemSettings().maxCount(10));
    }
...
//            user.addStatusEffect(new StatusEffectInstance(Z7StatusEffects.STATUS_EFFECT_HEALING, 280));
    ...
    public TypedActionResult<ItemStack> use(World world, PlayerEntity user, Hand hand) {
        if (user.hasStatusEffect(Z7StatusEffects.STATUS_EFFECT_CONCUSSION) || user.hasStatusEffect(Z7StatusEffects.STATUS_EFFECT_CONCUSSION_LONG) || user.getMaxHealth() != user.getHealth()) {
            ...
            user.sendMessage(Text.of("Unable to use painkillers: not concussed and full health."), true);
...
//        PotionUtil.buildTooltip(statusEffects, tooltip, 1);
        tooltip.add(MutableText.of(Text.of("Feel nothing...").getContent()).formatted(Formatting.GRAY));
        tooltip.add(ScreenTexts.EMPTY);
        tooltip.add(Text.translatable("tooltip.whenApplied").formatted(Formatting.DARK_PURPLE));
        tooltip.add(Text.translatable("attribute.modifier.plus.0", ItemStack.MODIFIER_FORMAT.format(HEAL_AMOUNT), Text.translatable(StatusEffects.INSTANT_HEALTH.getTranslationKey())).formatted(Formatting.BLUE));
