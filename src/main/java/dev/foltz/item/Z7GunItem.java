...
public abstract class Z7GunItem extends Item {
    ...
    public final Z7AmmoCategory ammoCategory;
    public final int firingTicks;
    public final int reloadingTicks;
    public final int ammoCapacity;
    private final Multimap<EntityAttribute, EntityAttributeModifier> attributeModifiers;

    public Z7GunItem(int maxDurability, int firingTicks, int reloadingTicks, int ammoCapacity, Z7AmmoCategory ammoCategory) {
    ...
    public abstract ItemStack beginShoot(ItemStack stack, World world, LivingEntity entity);
...
//        System.out.println("FIRING TICKS DIFF = " + diff);
    ...
    public ItemStack tickReload(ItemStack stack, World world, LivingEntity entity) {
        ...
        stack = loadOneBullet(stack, ammoStack);
...
//            int reloadStage = getAmmoInGun(stack).size() / reloadingTicks;
            int reloadStage = Math.round(MathHelper.lerp((float) getAmmoInGun(stack).size() / (float) ammoCapacity, 0.0f, reloadingTicks));
    ...
    public int findAmmoSlot(PlayerEntity player, Z7AmmoCategory ammoCategory) {
    ...
    public ItemStack loadOneBullet(ItemStack gun, ItemStack bulletStack) {
        List<ItemStack> ammoInGun = new ArrayList<>(getAmmoInGun(gun));
        if (ammoInGun.size() >= ammoCapacity) {
            return gun;
        }
        else {
            ItemStack bullet = bulletStack.split(1);
    ...
    public void appendTooltip(ItemStack stack, @Nullable World world, List<Text> tooltip, TooltipContext context) {
        ...
        tooltip.add(MutableText.of(Text.of("Ammo " + ammoList.size() + "/" + ammoCapacity + ":").getContent()).formatted(Formatting.GRAY));

        if (context.isAdvanced()) {
            ammoList.forEach(a -> {
                MutableText text = MutableText.of(Text.of("  - ").getContent()).append(Text.translatable(a.getTranslationKey()));
