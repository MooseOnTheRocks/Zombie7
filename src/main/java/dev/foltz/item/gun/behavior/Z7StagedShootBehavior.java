...
public class Z7StagedShootBehavior implements Z7IShootBehavior {
    ...
    public Z7StagedShootBehavior(List<Integer> preFireStages, boolean retainPrefireStage, int readyToFireOnReleaseStage, int readyToFireStage) {
    ...
    public void tickShootInventory(ItemStack stack, World world, Entity entity, int slot, boolean selected) {
        ...
        else if (!selected && gunlike.getGunStage(stack) != stageDefault && !retainPrefireStage && preFireStages.contains(gunlike.getGunStage(stack))) {
            gunlike.setGunStage(stack, stageDefault);
            gunlike.resetUsageStage(stack);
        }
    }
