...
public class StagedGunItem extends StagedItem {
    ...
    public StagedGunItem(int maxDurability, Z7AmmoItem.AmmoCategory ammoCategory, int maxAmmoCapacity, Map<String, GunStageBuilder> stagesMap) {
        super(new FabricItemSettings().maxDamage(maxDurability), new GunStageGraphBuilder(stagesMap).build());
        this.ammoCategory = ammoCategory;
        this.maxAmmoCapacity = maxAmmoCapacity;
    }

    public StagedGunItem(int maxDurability, Z7AmmoItem.AmmoCategory ammoCategory, int maxAmmoCapacity, GunStageGraph graph) {
        super(new FabricItemSettings().maxDamage(maxDurability), graph);
        this.ammoCategory = ammoCategory;
        this.maxAmmoCapacity = maxAmmoCapacity;
    }

    public static GunStageEventHandler tryFireReset(String stageShooting) {
        return view -> {
            if (view.isPressingShoot() && view.gun.hasAmmoInGun(view.stack)) {
                view.gun.setStageTicks(view.stack, 0);
    ...
    public static GunStageEventHandler tryShootOrReloadInit(String stageShooting, String stageReloading) {
        return view -> {
//            System.out.println("tryShootOrReloadInit: " + view.isPressingShoot() + " | " + view.isPressingReload());
    ...
    public static GunStageEventHandler tryShootOrReload(String stageShooting, String stageReloading) {
        return view -> {
            if (view.isPressingShoot() && view.gun.hasAmmoInGun(view.stack)) {
    ...
    public static GunStageEventHandler tryReloadInit(String stageReloading) {
        return view -> {
            if ((view.isPressingShoot() || view.isPressingReload()) && view.gun.hasRoomInGun(view.stack) && view.gun.hasAmmoInInventory(view.entity)) {
    ...
    public static GunStageEventHandler tryReload(String stageReloading) {
        return view -> {
            if ((view.isPressingShoot() || view.isPressingReload()) && view.gun.hasRoomInGun(view.stack) && view.gun.hasAmmoInInventory(view.entity)) {
                return stageReloading;
            }
    ...
    public static GunStageEventHandler tryReloadWholeClip(String stageDefault) {
        return view -> {
            var stack = view.stack;

            if (view.entity instanceof PlayerEntity player) {
                for (int i = view.gun.getAmmoInGun(stack).size(); i < view.gun.getMaxAmmoCapacity(stack); i++) {
                    int slot = view.gun.ammoCategory.findAmmoSlot(player);
                    if (slot == -1) {
                        break;
                    }
                    ItemStack ammoStack = player.getInventory().getStack(slot);
                    view.gun.loadOneBullet(stack, ammoStack, !player.getAbilities().creativeMode);
                }
            }
            else {
                for (int i = view.gun.getAmmoInGun(stack).size(); i < view.gun.getMaxAmmoCapacity(stack); i++) {
                    view.gun.loadOneBullet(stack, view.gun.ammoCategory.defaultItem.get().getDefaultStack(), false);
                }
            }

            view.gun.playSoundReloadEnd(stack, view.entity);
            return stageDefault;
        };
    }

    public static GunStageEventHandler tryReloadOneBullet(String stageDefault) {
        return view -> {
            var stack = view.stack;
            ...
            int usageTicks = view.gun.getStageTicks(stack);
            int maxStageTicks = view.gun.getMaxStageTicks(stack);
            int loadingAmmoStage = (int) (view.gun.getMaxAmmoCapacity(stack) * (usageTicks / (float) maxStageTicks));
            ...
            if (usageTicks >= maxStageTicks) {
                ItemStack ammoStack = player.getInventory().getStack(slot);
                view.gun.loadOneBullet(stack, ammoStack, !player.getAbilities().creativeMode);
                view.gun.playSoundReloadEnd(stack, player);
                return stageDefault;
            }
            // Loading a bullet.
            else if (loadingAmmoStage > view.gun.getAmmoInGun(stack).size()) {
                ItemStack ammoStack = player.getInventory().getStack(view.gun.ammoCategory.findAmmoSlot(player));
                view.gun.loadOneBullet(stack, ammoStack, !player.getAbilities().creativeMode);
                view.gun.playSoundReloadStep(stack, player);
            }
    ...
    public static GunStageEventHandler doFire() {
        return view -> {
            view.gun.shootGun(view.stack, view.world, view.entity);
            view.gun.playSoundFireBegin(view.stack, view.entity);
            return view.stageId;
        };
    }

    public static GunStageEventHandler doCancel(String stageDefault) {
        return view -> {
            view.gun.playSoundReloadCancel(view.stack, view.entity);
    ...
    public static GunStageEventHandler doReady(String stageReady) {
        return view -> {
            view.gun.playSoundReadyToFire(view.stack, view.entity);
            return stageReady;
        };
    }
...
//
//    public static final List<String> STAGES_READYING = List.of(STAGE_COCKING);
//    public static final List<String> STAGES_READY_TO_FIRE = List.of(STAGE_COCKED);
//    public static final List<String> STAGES_FIRING = List.of(STAGE_FIRING);
//    public static final List<String> STAGES_RELOADING = List.of(STAGE_RELOADING);
//
    public boolean predicateIsReloading(ItemStack stack) {
        return false;
    }

    public boolean predicateIsReadying(ItemStack stack) {
        return false;
    }

    public boolean predicateIsReadyToFire(ItemStack stack) {
        return false;
    }

    public boolean predicateIsFiring(ItemStack stack) {
        return false;
    }
