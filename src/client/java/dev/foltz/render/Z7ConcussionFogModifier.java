...
public  class Z7ConcussionFogModifier implements BackgroundRenderer.StatusEffectFogModifier {
    @Override
...
//        float f = MathHelper.lerp(Math.min(1.0f, (float) effect.getDuration() / 20.0f), viewDistance, 5.0f);
        ...
        float f = effect.isDurationBelow(s - 1)
                ? d + (Z7ConcussionLongFogModifier.FOG_DIST_FACTOR * 1.15f - d) * (float) Math.pow((1 - Math.pow(effect.getDuration(), n) / Math.pow(s, n)), m)
                : d;
